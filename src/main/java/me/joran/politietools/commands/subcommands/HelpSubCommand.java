...
public class HelpSubCommand {
    ...
    private String permission = FileManager.get("config.yml").getString("permissions.help");

    public void onCommand(CommandSender sender, String label, String[] args) {
        Player player = (Player) sender;

        if (args.length == 0) {
            if (player.hasPermission(permission)) player.sendMessage(ChatUtils.format("&a" + Icon.CHECKMARK + " /112, 100, 101, 102 <bericht>"));
            if (player.hasPermission(permission)) player.sendMessage(ChatUtils.format("&a" + Icon.CHECKMARK + " /politietools <help>"));
            if (!player.hasPermission(permission)) player.sendMessage(ChatUtils.format("&c" + Icon.CROSS + " /politietools <help>"));
        } else {
